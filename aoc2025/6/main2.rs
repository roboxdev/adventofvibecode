...
fn solve_puzzle(input: &str) -> u64 {
    ...
    // Identify problem boundaries by finding columns that are entirely spaces
    ...
    for (start_col, end_col) in problems {
        ...
        // For Part 2: Each column within the problem represents a number
        // Read each column top-to-bottom, concatenating digits to form the number
        let mut numbers: Vec<u64> = Vec::new();

        for col in start_col..end_col {
            // Skip if this column is only the operator (no digits in number rows)
